...
static NSDictionary *sWhitelistPList;
...
void OOSanitizerSetWhitelist(NSDictionary *whitelist)
{
	if (sWhitelistPList != whitelist)
	{
		[sWhitelistPList release];
		sWhitelistPList = [whitelist retain];
	}
}
...
static NSArray *SanitizeCondition(NSString *condition, SanStackElement *stack)
{
	...
	tokens = OOScanTokensFromString(condition);
...
static NSArray *SanitizeActionStatement(NSString *statement, SanStackElement *stack, BOOL allowAIMethods)
{
	...
	tokens = OOScanTokensFromString(statement);
	tokenCount = [tokens count];
	...
	return [NSArray arrayWithObjects:[NSNumber numberWithBool:NO], statement, selectorString, argument, nil];
}
...
static NSString *SanitizeQueryMethod(NSString *selectorString)
{
	...
	if (whitelist == nil)
	{
		whitelist = [[NSSet alloc] initWithArray:[sWhitelistPList oo_arrayForKey:@"query_methods"]];
		aliases = [[sWhitelistPList oo_dictionaryForKey:@"query_method_aliases"] retain];
	}
...
static NSString *SanitizeActionMethod(NSString *selectorString, BOOL allowAIMethods)
{
	...
	if (whitelist == nil)
	{
		...
		actionMethods = [sWhitelistPList oo_arrayForKey:@"action_methods"];
		aiMethods = [sWhitelistPList oo_arrayForKey:@"ai_methods"];
		aiAndActionMethods = [sWhitelistPList oo_arrayForKey:@"ai_and_action_methods"];
		...
		aliases = [[sWhitelistPList oo_dictionaryForKey:@"action_method_aliases"] retain];
		
		aliasesWithAI = [sWhitelistPList oo_dictionaryForKey:@"ai_method_aliases"];
